...
13. Получить название карточки 
14. Получить количество 
15. Получить состояние
16. Получить поставщика
17. Получить производителя
18. Получить стоимость
19. Получить адрес
20. Получить город
21. Получить срок годности
22. Получить категорию
23. Получить примечание
24. Назад
'''

    user_choice = 0

    while user_choice != 24:
        try:
            user_choice = int(input(menu))

            if user_choice not in range(1, 25):
                print('Выберите число от 1 до 24')
        except ValueError:
        ...
        try:
            match user_choice:
                ...
                case 13:
                    print(product.get_dict()['Наименование'])
                case 14:
                    print(product.get_dict()['Количество'])
                case 15:
                    print(product.get_dict()['Состояние'])
                case 16:
                    print(product.get_dict()['Поставщик'])
                case 17:
                    print(product.get_dict()['Производитель'])
                case 18:
                    print(product.get_dict()['Стоимость'])
                case 19:
                    print(product.get_dict()['Местоположение'])
                case 20:
                    print(product.get_dict()['Город'])
                case 21:
                    print(product.get_dict()['Срок годности'])
                case 22:
                    print(product.get_dict()['Категория'])
                case 23:
                    print(product.get_dict()['Заметка'])
...
if __name__ == '__main__':
    txt = TxtInteract('data')
    cards = txt.deserialize()
...
'''
    ...
    while user_card_choice != 3:
        ...
        try:
            ...
            if user_card_choice not in range(1, 5):
                raise ValueError('Выберите число от 1 до 4')
        except ValueError as ve:
...
Начните заполнение карточки, выберите одно из действий, описанных ниже: ''')
            ...
            case 4:
                try:
                    del_choice = int(input('Введите номер карточки для удаления: '))

                    json_file.delete_card(del_choice)

                    if str(del_choice) in dict_of_objects:
                        del dict_of_objects[str(del_choice)]

                        print('Карточка удалена ')
                    else:
                        print('Карточка с таким номером отсутствует')
                except ValueError:
                    print('Необходимо ввести целое число')
